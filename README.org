...
[[file:screencast.gif][Watch it in action!]]
...
Once things stabilize a bit and I'll package this for MELPA.
...
package managers support custom build steps and so it should be simple to add a
recipe to build the module to them. I'm currently a bit hesitant to add lisp
code to do this automagically or fetch prebuilt modules from the web like
~pdf-tools~ or ~emacs-libvterm~, because I'm a believer that it should be the
job of a package manager, but perhaps I'll be convinced otherwise.
